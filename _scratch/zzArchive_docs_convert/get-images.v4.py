...
def replaceImageXrefInMarkdown(newMarkdownContent, imageURL, imageFileNameFull):

    new_file_content = ""
    for line in newMarkdownContent:
...
def updateImageRefsInFile(mdFileName): 
    ...
    for i, line in enumerate(open(mdFileName)):
        for match in re.finditer(_imageURLpattern, line):
            ...
            newMarkdownContent = replaceImageXrefInMarkdown(mdf, imageURL, imgTarget)
    mdf.write(newMarkdownContent)
    mdf.close()
