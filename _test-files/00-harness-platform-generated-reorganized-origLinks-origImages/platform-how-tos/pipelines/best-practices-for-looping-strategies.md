...
A good general rule to follow is: **Your looping scenario is too complex if you cannot visualize how it will run and calculate the memory and CPU required for the Pipeline to run successfully.**### How Pipelines Reserve Resources
...
* Each Step consumes up to 500M (memory) and 400m (CPU). Because the Steps run serially, not concurrently, the Pipeline reserves 500Mi memory and 400m CPU for the entire Stage.![](https://files.helpdocs.io/kw8ldg1itf/articles/q7i0saqgw4/1658090470698/resource-scenario-00.png)
...
matrix:  
  ...
  maxConcurrency: 2 
```
* The Pipeline creates two copies of the Run Stage and runs them concurrently. This doubles the resource consumption for the overall Stage. When the Pipeline runs, it reserves double the resources (1000M memory, 800m CPU) for the overall Stage.![](https://files.helpdocs.io/kw8ldg1itf/articles/q7i0saqgw4/1658090508676/resource-scenario-01.png)
...
* [How to Run a Run a Stage or Step Multiple Times using a Matrix](https://docs.harness.io/article/kay7z1bi01)
