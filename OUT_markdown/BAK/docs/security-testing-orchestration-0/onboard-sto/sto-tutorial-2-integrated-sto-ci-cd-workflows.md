...
This tutorial builds on the [previous tutorial](https://docs.harness.io/article/yvy4pmt8bw). You need to complete Tutorial 1 first.
...
Every Security Step supports a `fail_on_severity` setting. If any vulnerability with the specified severity or higher is found, the Pipeline fails. It is good practice to include this setting in every Security Step in an integrated Pipeline.1. In the Pipeline Studio, open the **STO Tutorial 1** pipeline > **SecurityTestStage** stage > **banditScan** step.
2. Under **Settings**, add the following: `fail_on_severity` = `CRITICAL`
3. Click **Apply Changes**, save the updated pipeline, and run a new build with the **DEMO-001** branch.![](https://files.helpdocs.io/kw8ldg1itf/articles/y22pgbobm5/1666724864917/tut-02-04-banditscan-failed-v-2.png)The pipeline now fails because the bandit step is now configured to fail on any vulnerability with a severity of low or higher. The last log message in the bandit step log is:
```
...
Developer users cannot create Ignore Rules; only SecOps users have this permission.Harness provides two pre-defined roles for STO:

* **Developer** role — Permissions needed for developer workflows. These workflows are described in [Tutorial 1](https://docs.harness.io/article/yvy4pmt8bw). A Developer can set up security pipelines, run scans, and view results. A Developer can also request (but not approve) Ignore rules for specific issues.
* **SecOps** role — Permissions needed for Security Operations staff. This role includes all Developer permissions. In addition, SecOps users can approve Ignore rules. These workflows are covered in [Tutorial 2](https://docs.harness.io/article/zy4h4ch6dh).
...
1. Make sure that you have the SecOps role assigned to yourself.Assign SecOps role to yourself
	...
	4. Make sure that you have the **Security Testing SecOps** role assigned to yourself.![](https://files.helpdocs.io/kw8ldg1itf/articles/y22pgbobm5/1666967042962/assign-secops-role.png)
...
3. In the **Security Tests** tab, do the following steps for each of the two critical issues:
	...
	2. Click the **Ignore** button.![](https://files.helpdocs.io/kw8ldg1itf/articles/y22pgbobm5/1666660833239/tut-02-06-ignore-butto.png)
	3. In the **Request to Ignore an Issue** popup, configure the Ignore request as follows:
		...
		4. Click **Create Request**.![](https://files.helpdocs.io/kw8ldg1itf/articles/y22pgbobm5/1666661208582/tut-02-07-request-to-ignore-issue.png)
4. Click **Security Tests** (left menu) and then **Security Review** (second-from-left menu).
5. In the Security Review page, click the "thumbs-up" buttons to approve both exemptions.![](https://files.helpdocs.io/kw8ldg1itf/articles/y22pgbobm5/1666661810872/tut-02-08-approve-exemptions.png)
...
7. In the issues table (bottom), each section has a set of show/hide buttons for different issue types: Critical, High, Medium. Low, and Info. Note that each section now includes an **Ignored** button. Also note that the ignored issues are hidden by default.![](https://files.helpdocs.io/kw8ldg1itf/articles/y22pgbobm5/1666727171944/tut-02-09-ignored-issue-buttons.png)
8. Click the **Ignored** buttons (right) and the expand/contract buttons (left) so that both ignored issues are visible.![](https://files.helpdocs.io/kw8ldg1itf/articles/y22pgbobm5/1666727453754/tut-02-10-ignored-issues-visible.png)
9. Go to **Security Tests** > **Security Review**. Then click **Approved** to show the Ignore rules you created and approved.
10. Click the Delete (**X**) buttons on the right to delete both rules.![](https://files.helpdocs.io/kw8ldg1itf/articles/y22pgbobm5/1666727818360/tut-02-11-delete-ignore-rules.png)
