...
You can also use an AWS VM build infrastructure. See [Define an AWS VM Build Infrastructure](/article/z56wmnris8-set-up-an-aws-vm-build-infrastructure).### Limitations
...
Autopilot clusters do not allow Privileged pods. Thus you cannot use [Docker-in-Docker](https://ngdocs.harness.io/article/ajehk588p4) to run Docker commands, since these require Privileged mode.
...
* Ensure your Kubernetes cluster meets the build infrastructure requirements in [CI Cluster Requirement](/article/sjjik49xww-kubernetes-cluster-connector-settings-reference#harness_ci_cluster_requirements).
