...
The current version of this SDK is **1****.0.3.**### Requirements
...
For more information about Targets, go to [Targeting Users With Flags](../ff-target-management/targeting-users-with-flags.md).To add a Target, build it using `CfTarget.builder` and pass in arguments for the following:
...
```
You also need to call this method when changing accounts on the `CfTarget` object to re-initialize and fetch Evaluations for the correct Target.### Additional options
