...
def topicIsPublished(metaDict):
    # print("[DEBUG] ", metaDict)
    print("[DEBUG] ", metaDict['article_id'], " is published: ", metaDict['is_published'])
    if (metaDict['is_published'] == True ):
        print("[DEBUG] returning True")
        return True
    else:
        print("[DEBUG1] returning False")
        return False
        
def topicIsPrivate(metaDict):
    print("[DEBUG] ", metaDict['article_id'], "is private: ", metaDict['is_private'])
...
for htmlFileName in glob.iglob(_htmlSourceRoot + '**/**', recursive=True):
    if htmlFileName.endswith('.html'):
         ...
         if (topicIsPublished(fmDict) == True):            
